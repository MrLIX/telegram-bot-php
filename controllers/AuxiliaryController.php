...
{
    public $basePhotoUrl = 'http://tripzy.uz/other/uploads/';
    ...
    /** Cart info for $answer
     * @param $allCart
     * @param $lang
     * @return string
     */
    public function getCartText($allCart, $lang)
    {
        $text = $lang == 'ru' ? '📥 В корзине:
' : '📥 Savetchada:
';
        $amount = 0;
...
' . $item['count'] . ' x ' . $item['price'] .' = ' . $item['amount'] .'
';
            }
            $amount += $item['amount'];
        }
...
Umumiy hisob' . $amount;
     ...
     * @return string
     */
    public function sendOrderToChannelText($order, $chatID)
    {
...
";
        ...
        foreach ($orderProduct as $item){
            $product = Products::findOne($item->product_id);
            $text .= $product->category->name_ru. ' - ' .$product->name_ru . '
' . $item->count . ' x ' . $item->price . ' = ' . $item->amount .' сум
';
        }
        $text .= "
<b>Общая сумма</b>: " . number_format($order->amount,0,' ',' ') . " сум
";
        // User Info
        $text .= "<code>---------</code>
<i>Имя:</i> ". $user->name . "
<i>Телефон:</i> +" . $order->phone . "
<i>Адрес:</i> " . $order->address . "
